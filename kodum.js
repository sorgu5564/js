// ============================================
// PEGASUS SMARTGO - v21.16
// Login sonrası otomatik başlatma
// ============================================
...
(function() {
    ...
    console.log('%c✈️ Pegasus SmartGO v21.16 - Login Bekleniyor...', 'color: #0066cc; font-size: 18px; font-weight: bold');
    ...
    function initPanel(token) {
        ...
        // ============ UÇAK TAKİP BUTONLARI ============
        function getFlightAwareButton(regSerial) {
            if (!regSerial || regSerial === '-' || regSerial === 'null') return '';
            return `<a href="https://tr.flightaware.com/live/flight/${regSerial.replace(/-/g,'')}" target="_blank" class="track-btn track-btn-fa">✈️ FlightAware</a>`;
        }
        
        function getFlightRadar24Button(regSerial) {
            if (!regSerial || regSerial === '-' || regSerial === 'null') return '';
            return `<a href="https://www.flightradar24.com/data/aircraft/${regSerial.replace(/-/g,'')}" target="_blank" class="track-btn track-btn-fr24">📡 FlightRadar24</a>`;
        }
        ...
        function playNotificationSound() {
            try {
                ...
                const playBeep = (freq, dur, start, vol = 0.7) => {
                ...
                for (let i = 0; i < 5; i++) playBeep(880, 0.15, i * 0.15, 0.8);
                setTimeout(() => { for (let i = 0; i < 2; i++) playBeep(660, 0.3, i * 0.35, 0.7); }, 800);
            } catch(e) {}
        }
        ...
        async function fetchLDM(legIsn) {
            try {
                let resp = await fetch(`https://smartgo.flypgs.com/api/movementMessages/getLdmMessage/${legIsn}`, {
                    method: 'GET',
                    headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' }
                });
                if (!resp.ok) return null;
                let data = await resp.json();
                if (data && data.ldmMessage) {
                    let raw = data.ldmMessage.ldmMessage || data.ldmMessage;
                    return { raw: raw, isLDM: true };
                }
                return null;
            } catch(e) { return null; }
        }
...
.track-buttons{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}
.track-btn{display:inline-flex;align-items:center;gap:5px;padding:6px 12px;border-radius:20px;text-decoration:none;font-size:10px;font-weight:600;transition:all .2s;color:#fff}
.track-btn-fa{background:linear-gradient(135deg,#1a5bbf,#004e9e)}
.track-btn-fr24{background:linear-gradient(135deg,#ff6b35,#c73e1d)}
.track-btn:hover{transform:scale(1.03);opacity:0.9}
...
.personnel-highlight{border:2px solid #1a5bbf!important;box-shadow:0 0 8px rgba(26,91,191,.3)!important}
...
<div class="app-header"><button class="close-btn" id="closeAppBtn">✕</button><h1>✈️ Pegasus SmartGO <small>v21.16</small></h1><div class="time-range"><span>🕐</span><span id="timeRangeText">${getTimeRangeText()}</span></div></div>
<div class="control-panel">
<div class="control-row"><div class="status-badge"><span class="status-dot" id="statusDot"></span><span id="statusText">Duruyor</span></div><input id="searchInput" class="search-input" placeholder="Personel adı/kodu..." value="SAHIN"><select id="hubSelect" class="hub-select"><option value="SAW">SAW</option><option value="AYT">AYT</option><option value="ESB">ESB</option><option value="ADB">ADB</option><option value="DLM">DLM</option></select></div>
...
<div class="log-area" id="logArea"><div class="log-entry">✅ v21.16 - Login sonrası otomatik</div></div>`;
            document.body.appendChild(app);
            
            document.getElementById('closeAppBtn').onclick = () => { clearAllCountdowns(); if(monitorInterval) clearInterval(monitorInterval); previousTasksMap.clear(); isFirstScan = true; app.remove(); };
            ...
            document.getElementById('notifyBtn').onclick = async () => {
                ...
                if(notificationEnabled) { document.getElementById('notifyBtn').style.background='#28a745'; document.getElementById('notifyBtn').style.color='white'; document.getElementById('notifyBtn').textContent='🔊 Aktif'; }
            };
            ...
            document.getElementById('searchInput').onchange = e => { currentConfig.searchValue = e.target.value; previousTasksMap.clear(); isFirstScan = true; if(isMonitoring) forceScan(); };
        ...
        function renderContent(flights, tasks, loadsheetMap, allTasksMap) {
            ...
            for (let { flight, countdown, flightNo, flightTasks, allFlightTasks } of flightsWithCountdown) {
                ...
                if (isValidValue(flight.Departure_Port)) {
                    let d = getAirportName(flight.Departure_Port);
                ...
                if (isValidValue(flight.Arrival_Port)) {
                    let d = getAirportName(flight.Arrival_Port);
                ...
                if (isValidValue(flight.RegSerial)) {
                    let t = getAircraftTypeName(flight.Departure_ACType || flight.Arrival_ACType);
                ...
                if (isValidValue(flight.Departure_CargoPiece) && isValidValue(flight.Departure_CargoKilo)) {
                    let c = `${flight.Departure_CargoPiece} adet / ${flight.Departure_CargoKilo} kg`;
                ...
                if (isValidValue(flight.Departure_STD)) { let t = addThreeHoursToTime(flight.Departure_STD); infoItems.push({label:'📅 PLAN. KALKIŞ', value: t ? t.display : flight.Departure_STD}); }
                if (isValidValue(flight.Arrival_STA)) { let t = addThreeHoursToTime(flight.Arrival_STA); infoItems.push({label:'📅 PLAN. VARIŞ', value: t ? t.display : flight.Arrival_STA}); }
                ...
                for (let t of allFlightTasks) {
                    ...
                    if (!personelMap.has(key)) personelMap.set(key, { tasks: [], code: t.assignedEmployeeCode || null, phone: t.assignedEmployeeMobilePhone || null });
                ...
                for (let [name, data] of personelMap) {
                    ...
                    let taskCards = data.tasks.map(task => {
                        let sc = task.taskStatusText || 'Waiting';
                ...
                html += `<div class="flight-card${dangerClass}"><div class="flight-card-header"><div class="flight-title"><div class="flight-number">${statusIcon}✈️ ${escapeHtml(flightNo)} <span class="flight-status">${statusLabels[flight.Status]||'?'}</span>${isValidValue(flight.RegSerial)?` <span class="flight-reg">${flight.RegSerial}</span>`:''}</div><span class="flight-badge">👥 ${totalPersonnel} | 📋 ${allFlightTasks.length}</span></div></div><div class="flight-card-body">${countdownHtml}<div class="info-grid">${infoItems.map(item => `<div class="info-item"><div class="info-label">${item.label}</div><div class="info-value ${item.className||''}">${item.value}</div></div>`).join('')}</div>${trackButtonsHtml}${loadsheetHtml}<div class="section-title">👥 PERSONEL (${totalPersonnel} kişi)</div>${tasksHtml}</div></div>`;
            }
        ...
        async function performScan() {
            ...
            try {
                ...
                let allTasks = [], loadsheetMap = new Map(), allTasksByFlight = new Map();
                for (let i = 0; i < legs.length; i += 2) {
                    let batch = legs.slice(i, i + 2);
                    try {
                        let [taskRes, loadsheetRes] = await Promise.all([
                            Promise.all(batch.map(l => fetchTasksForLeg(l).catch(e => []))),
                            Promise.all(batch.map(l => fetchLoadsheet(l).catch(e => null)))
                        ]);
                        batch.forEach((leg, j) => {
                            ...
                            else fetchLDM(leg).then(ldm => { if (ldm) loadsheetMap.set(leg, ldm); });
                        });
                    } catch(batchError) {}
                }
                ...
                allTasks.forEach(t => { if (isValidValue(t.assignedEmployeeName)) personelSet.add(t.assignedEmployeeName); if (t.taskName && t.taskName.toLowerCase().includes('ykb')) ykb++; });
        ...
        createAppInterface();
        addLog('✅ v21.16 - Login sonrası otomatik başlatma');
        addLog('📡 FlightRadar24 ve FlightAware linkleri');
    }
    
    // ============ TOKEN KONTROLÜ VE BAŞLATMA ============
    let existingToken = findToken();
    
    if (existingToken) {
        // Token zaten varsa hemen başlat
        console.log('✅ Token hazır, panel başlatılıyor...');
        setTimeout(() => initPanel(existingToken), 500);
    } else {
        // Token yoksa bekle
        console.log('⏳ Login bekleniyor... (Panel otomatik açılacak)');
        checkInterval = setInterval(() => {
            let token = findToken();
            if (token && !panelStarted) {
                initPanel(token);
            }
        }, 2000);
        
        // 10 dakika sonra durdur
        setTimeout(() => {
            if (checkInterval && !panelStarted) {
                clearInterval(checkInterval);
                console.log('⏰ Zaman aşımı - sayfayı yenileyip tekrar deneyin');
            }
        }, 600000);
    }
